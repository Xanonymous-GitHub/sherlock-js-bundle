...
function submissionResultsPage() {
  ...
  if ($('#compare-data').length || $('#report-data').length) {
    ...
    function rgba(colour) {
      return `rgba(${Number.parseInt(colour.slice(-6, -4), 16)},${Number.parseInt(colour.slice(-4, -2), 16)},${Number.parseInt(colour.slice(-2), 16)},0.4)`
    }
    ...
    function highlight(fileId, lines, colour) {
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        if (first) {
          ...
          const position = Number.parseInt(line.css('top'), 10)
    ...
    function bind() {
      ...
      $('[data-js=\'match-previous\']').on('click', (e) => {
        let previous = Number.parseInt(active, 10) - 1
      ...
      $('[data-js=\'match-next\']').on('click', (e) => {
        let next = Number.parseInt(active, 10) + 1
      ...
      $('[data-js=\'match-hide\']').on('click', () => {
        hideAll()
      })

      $('[data-js=\'matches-left\']').off()
      $('[data-js=\'matches-left\']').on('click', () => {
      ...
      $('[data-js=\'matches-right\']').on('click', () => {
      ...
      $('[data-js=\'matches-restore\']').on('click', () => {
...
function networkGraphPage() {
  if ($('[data-js=\'networkArea\']').length) {
    ...
    function addNode(id) {
      const result = $.grep(json.nodes, (n) => {
        return n.id === id
      })
    ...
    function addEdge(id1, id2) {
      const edge1 = $.grep(json.matches, (n) => {
        return n.to === id1 && n.from === id2
      })

      const edge2 = $.grep(json.matches, (n) => {
    ...
    function addMatches(id) {
      const to = $.grep(json.matches, (n) => {
        return n.to === id
      })

      const from = $.grep(json.matches, (n) => {
        return n.from === id
      })
 ...
 */
$(() => {
  $('form[data-js="autoSubmit"]').trigger('submit') // automatically submits the login page when running locally
